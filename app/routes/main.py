...
def video_stream():
    """Endpoint MJPEG: el <img> del HTML apunta aqui para el video en vivo."""
    return Response(
        _stream_generator(),
...
def estado_actual():
    """Retorna el estado actual de la prediccion como JSON (lo consulta el frontend cada 500ms)."""
    with _estado_lock:
        sena = _estado.get("sena_actual")
        confianza = _estado.get("confianza_actual", 0.0)
        mano = _estado.get("mano_detectada", False)
...
@main_bp.route("/enviar_telegram", methods=["POST"])
def enviar_telegram():
    """Recibe la sena detectada y la envia al bot de Telegram si esta habilitado."""
    data = request.get_json()
    sena = data.get("sena", "")
    confianza = data.get("confianza", 0.0)

    if not sena:
        return jsonify({"exito": False, "mensaje": "No hay sena para enviar"})

    config = cargar_config()

    if not config.get("telegram_activo", False):
        return jsonify({"exito": False, "mensaje": "El envio a Telegram esta desactivado. Activalo en el panel de administracion."})

    formato = config.get("formato_mensaje", "Sena: {sena} ({confianza:.0%})")
    try:
        mensaje = formato.format(sena=sena, confianza=confianza)
    except KeyError:
        mensaje = f"Sena detectada: {sena} ({confianza:.0%})"

    chat_id = config.get("telegram_chat_id", "")
    exito, descripcion = telegram_bot.enviar_mensaje(mensaje, chat_id)

    if config.get("historial_habilitado", True):
        metricas.registrar(sena, confianza, enviado_telegram=exito,
                           max_registros=config.get("max_historial", 200))
